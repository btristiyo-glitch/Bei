...
def morning_scan():
    ...
    results_reversal = []
    failed_tickers = []
    all_setups = []  # buat alert intraday
    ...
    for setup in setups:
        ...
        try:
            ...
            if low_today <= entry <= current:
                ...
                # Hapus dari alert list biar nggak ngirim ulang
                setups = [s for s in setups if s["ticker"] != setup["ticker"]]

        except Exception:
            continue

    # Simpan ulang (hapus yang sudah ke-trigger)
    with open(ALERT_FILE, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["ticker", "entry_limit", "tp1", "tp2", "tp3", "stop_loss", "score", "tipe"])
        for s in setups:
...
# ── JADWAL ──
# Pagi: scan jam 8:30 (pre-market) + jam 9:15 (30 menit setelah open)
...
# Alert: cek tiap 15 menit selama market buka (09:00 - 16:00)
...
# ── JALANKAN ──
morning_scan()  # Langsung scan sekali pas start
