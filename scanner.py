        ...
        for chat_id in CHAT_IDS:
            for attempt in range(3):
                try:
                    ...
                    if attempt == 2:
                        print(f"Telegram error ke chat_id {chat_id}: {e}")
                    time.sleep(2)
...
def level_entry(item, price, open_today, support):
    gap = safe_float(item.get("gap_pct"), 0) or 0
    tipe = item.get("tipe", "breakout")

    if tipe == "breakout":
        if gap > GAP_FILTER_PCT:
            entry_limit = max(support if support else price * 0.99, price * 0.99)
        elif gap > 0:
            entry_limit = min(open_today if open_today else price, price * 0.995)
        else:
            entry_limit = price * 0.995

        stop = min(support if support else price * 0.98, price * 0.98)
        ...
        entry_limit = support if support and support < price else price * 0.99
        stop = min((support * 0.99) if support else price * 0.96, price * 0.96)
        tp1 = price * 1.03
        tp2 = price * 1.06
        return (
            round(entry_limit, 0),
...
def init_csv_files():
    ...
    with open(ALERT_FILE, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "stop_loss", "score", "tipe", "scan_time", "gap_pct"
        ])
...
def morning_scan():
    ...
    for i, stock in enumerate(stocks):
        ...
        try:
            ...
            if score_m >= 40:
                el, sl, tp1, tp2, tp3 = level_entry(item, price, item["open_today"], item["support"])
                item["entry_limit"] = el
                item["stop_loss"] = sl
                item["tp1"] = tp1
                item["tp2"] = tp2
            ...
            if score_r >= 25:
                ...
                el, sl, tp1, tp2, tp3 = level_entry(item_rev, price, item_rev["open_today"], item_rev["support"])
    ...
    for item in results_momentum[:8]:
        ...
        gap_warning = " ⚠️ GAP >5% - TUNGGU PULLBACK KE SUPPORT" if item["gap_pct"] > GAP_FILTER_PCT else ""
    ...
    for setup in setups:
        ...
        if scan_time_str:
            try:
                ...
                if (now - scan_dt) > timedelta(hours=ALERT_EXPIRY_HOURS):
                    print(f"  ⏰ {setup.get('ticker')} expired")
                    continue
            except Exception:
                pass
        ...
        try:
            ...
            if low_today <= entry <= current:
                gap_scan = safe_float(setup.get("gap_pct"), 0) or 0
                gap_msg = "\n⚠️ Gap saat scan >5% - pastikan ini pullback valid" if gap_scan > GAP_FILTER_PCT else ""
