...
def compute_stop_loss(df, entry_price, gap_pct):
    try:
        ...
        # Stop loss = di bawah swing low + buffer ATR
        buffer = 0.5 * atr_val
        stop_loss = swing_low_10 - buffer

        # Kalau entry terlalu dekat dengan stop, longgarkan sedikit
...
def level_entry_breakout(item, price, open_today, support, df):
    ...
    # Entry di area support / retest, bukan kejar harga
...
def morning_scan():
    ...
    msg1 = "🔥 **BREAKOUT MOMENTUM - GAP 2-5% Wajib**\n_ATR + swing low stop loss_\n\n"
        ...
        try:
            ...
            if low_today <= entry <= current:
                msg = (
                    ...
                    f"Current: Rp {current:,.0f} | Tipe: {signal.upper()}\n"
                    f"┃ Stop: Rp {stop:,.0f}\n"
                    f"┃ TP1: Rp {tp1:,.0f}\n"
                    f"┃ Score: {score}\n"
