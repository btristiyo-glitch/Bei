...
def level_entry(item, price, open_today, support, rvol, atr_pct, scan_time):
 # ── GAP FILTER: kalau gap > 5%, entry bukan di open, tunggu pullback ──
 gap = item.get("gap_pct", 0)
 if gap > 5:
 # Entry di support intraday (pullback level), bukan di open
 ...
 entry_limit = support if support < price else price * 0.99
 stop = min(support * 0.99, price * 0.97)
 tp1 = price * 1.03
 tp2 = price * 1.06
...
def morning_scan():
 ...
 # ── HAPUS CSV LAMA biar signal kemarin nggak kepake ──
 if os.path.exists(CSV_FILE):
 os.remove(CSV_FILE)
 print("  🗑️ CSV lama dihapus")
 ...
 "scan_time": scan_time  # ── SIMPAN waktu scan ──
 }

 # Momentum
 score_m = 0
 if rvol > 5: score_m += 35
 elif rvol > 3: score_m += 20
 elif rvol > 2: score_m += 10
 if gap_pct is not None and gap_pct > 3: score_m += min(30, gap_pct * 4)
 elif gap_pct is not None and gap_pct > 0: score_m += 8
 if breakout_high: score_m += 15
 if rsi_now > 50 and rsi_now > rsi_prev: score_m += 10
 elif rsi_now > 40 and rsi_now > rsi_prev: score_m += 5
 if 3 < range_pct < 25: score_m += 10
 elif range_pct >= 25: score_m += 5
 if "🟢 BULLISH" in IHSG_line: score_m += 10
 if price > ema20: score_m += 5
 if ema20 > ema50: score_m += 5

 item["tipe"] = "breakout"
 item["score"] = round(score_m, 1)
 if score_m >= 40:
 el, sl, tp1, tp2, tp3 = level_entry(item, price, item["open_today"],
 item["support"], rvol, atr_pct, scan_time)
 item["entry_limit"] = el
 item["stop_loss"] = sl
 item["tp1"] = tp1
 item["tp2"] = tp2
 ...
 # Reversal
 score_r = 0
 if rsi_now < 22: score_r += 30
 elif rsi_now < 28: score_r += 20
 elif rsi_now < 35: score_r += 10
 score_r += max(0, min(20, (rsi_now - rsi_prev) * 5))
 score_r += min(15, rvol * 3)
 if price < ema20: score_r += 10
 if not breakout_high: score_r += 10
 if fundament_flag: score_r += 5
 if "🟢 BULLISH" in IHSG_line: score_r += 5

 if score_r >= 25:
 item_rev = item.copy()
 item_rev["tipe"] = "reversal"
 item_rev["score"] = round(score_r, 1)
 el, sl, tp1, tp2, tp3 = level_entry(item_rev, price, item_rev["open_today"],
 item_rev["support"], rvol, atr_pct, scan_time)
 ...
 w = csv.writer(f)
 w.writerow(["ticker", "entry_limit", "tp1", "tp2", "tp3", "stop_loss",
 "score", "tipe", "scan_time"])
 for item in all_setups:
 w.writerow([item["ticker"], item["entry_limit"], item["tp1"],
 ...
 # Tampilkan info gap kalau > 5%
 gap_warning = ""
 if item["gap_pct"] > 5:
 gap_warning = " ⚠️ GAP >5% — TUNGGU PULLBACK KE SUPPORT"
 ...
 msg1 += (
 f"{gap_emoji} **#{item['ticker']}**{funda}{gap_warning} | Score: {item['score']}\n"
 ...
 if len(msg1) > 3800: break
 if len(msg1) > 200: send(msg1)
 ...
 if len(msg2) > 3800: break
 if len(msg2) > 200: send(msg2)
...
def check_alerts():
 ...
 # ── GAP FILTER: kalau gap > 5% pas scan, entry cuma di support ──
 # (udah di-handle di level_entry, tapi extra safety di sini)
 gap_scan = setup.get("gap_pct", 0)
 try:
 gap_scan = float(gap_scan)
 except:
 gap_scan = 0
 ...
 gap_msg = ""
 if gap_scan > 5:
 gap_msg = "\n⚠️ Gap saat scan >5% — pastiin udah pullback"

 msg = (
 f"🚨 **ALERT ENTRY – #{setup['ticker']}**{gap_msg}\n"
 f"Harga menyentuh entry limit: Rp {entry:,.0f}\n"
 f"Current: Rp {current:,.0f} | Tipe: {tipe.upper()}\n"
 f"┃ Stop: Rp {stop:,.0f}\n"
 f"┃ TP1: Rp {tp1:,.0f}\n"
 f"┃ Score: {score}\n"
 f"Chart: https://www.tradingview.com/chart/?symbol=IDX:{setup['ticker']}\n"
 f"{datetime.now().strftime('%H:%M')} WIB"
 )
 send(msg)
 # Udah trigger, jangan masukin ke active_setups
 continue
 ...
 # ── Simpan ulang (hapus yang expired & sudah trigger) ──
 with open(ALERT_FILE, "w", newline="", encoding="utf-8") as f:
 w = csv.writer(f)
 w.writerow(["ticker", "entry_limit", "tp1", "tp2", "tp3", "stop_loss",
 "score", "tipe", "scan_time"])
 for s in active_setups:
 w.writerow([s["ticker"], s["entry_limit"], s["tp1"],
 s["tp2"], s["tp3"], s["stop_loss"],
 s["score"], s["tipe"],
 s.get("scan_time", "")])

 print(f"  ✅ Alert check selesai — {len(active_setups)} aktif, {len(setups)-len(active_setups)} dihapus")
